                          ...
                          x509_name *cur )
{
    ...
    while ( 1 ) {
        ...
        do
        {
            ...
            if( *p != end )
            {
                use->next = (x509_name *) kmalloc(
                        sizeof( x509_name ), GFP_KERNEL );
...
//             return( POLARSSL_ERR_X509_CERT_INVALID_DATE );
        time->year = (date[0] - '0')*10 + date[1] - '0';
        time->mon = (date[2] - '0')*10 + date[3] - '0';
        time->day = (date[4] - '0')*10 + date[5] - '0';
        time->hour = (date[6] - '0')*10 + date[7] - '0';
        time->min = (date[8] - '0')*10 + date[9] - '0';
        time->sec = (date[10] - '0')*10 + date[11] - '0';
                             ...
                             x509_crl_entry *entry )
{
    ...
    while( *p < end )
    {
        ...
        if ( *p < end )
        {
            cur_entry->next = kmalloc( sizeof( x509_crl_entry ), GFP_KERNEL );
...
int x509parse_crt_der( x509_cert *crt, const unsigned char *buf, size_t buflen )
{
    ...
    p = (unsigned char *) kmalloc( len = buflen, GFP_KERNEL );
...
int x509parse_crt( x509_cert *chain, const unsigned char *buf, size_t buflen )
{
    ...
    {
        crt->next = (x509_cert *) kmalloc( sizeof( x509_cert ), GFP_KERNEL );
...
// #endif
    ...
    {
        crl->next = (x509_crl *) kmalloc( sizeof( x509_crl ), GFP_KERNEL );
...
//     fseek( f, 0, SEEK_SET );
// 
//     if( ( *buf = (unsigned char *) kmalloc( *n + 1 , GFP_KERNEL) ) == NULL )
//         return( POLARSSL_ERR_X509_MALLOC_FAILED );
// 
...
//         sec   > to->sec  )
//         return( 1 );
    unsigned long valid_to;
    struct timespec now;

    valid_to = mktime(to->year, to->mon, to->day, to->hour, to->min, to->sec);

    getnstimeofday(&now);

    if(now.tv_sec > valid_to)
         return( 1 );
