...
//     md5_finish( &md5_ctx, md5sum );
    md_starts( &md5_ctx );
    md_update( &md5_ctx, pwd, pwdlen );
    md_update( &md5_ctx, iv,  8 );
    md_finish( &md5_ctx, md5sum );
                               ...
                               const unsigned char *pwd, size_t pwdlen )
{
    ...
    des_init_ctx(&des_ctx);
    des_setkey_dec( &des_ctx, des_key );
    des_crypt_cbc( &des_ctx, 0, buflen,
                     des_iv, buf, buf );

    // memset( &des_ctx, 0, sizeof( des_ctx ) );
    des_free_ctx(&des_ctx);
    memset( des_key, 0, 8 );
}
                               ...
                               const unsigned char *pwd, size_t pwdlen )
{
    ...
    des3_init_ctx(&des3_ctx);
    des3_set3key_dec( &des3_ctx, des3_key );
    des3_crypt_cbc( &des3_ctx, 0, buflen,
                     des3_iv, buf, buf );

    // memset( &des3_ctx, 0, sizeof( des3_ctx ) );
    des3_free_ctx(&des3_ctx);
                               ...
                               const unsigned char *pwd, size_t pwdlen )
{
    ...
    aes_init_ctx(&aes_ctx);
    aes_setkey_dec( &aes_ctx, aes_key, keylen * 8 );
    aes_crypt_cbc( &aes_ctx, 0, buflen,
                     aes_iv, buf, buf );
...
// #endif /* POLARSSL_MD5_C && (POLARSSL_AES_C || POLARSSL_DES_C) */
    ...
    if( ( buf = (unsigned char *) kmalloc( len, GFP_KERNEL ) ) == NULL )
